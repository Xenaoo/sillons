...
- Les fluctuations temporaires du marché et des événements continuent d’alimenter le calcul économique interne, mais elles ne sont plus mélangées avec la demande annuelle affichée.
- Même séparation appliquée à la demande fret affichée pour éviter le même effet visuel.
- Incrément de version : interface et serveur v0.70.12, package npm 0.70.12.

# v0.70.11 — correctif sillons et voyageurs transportés

- Correction du calcul des sillons exploités : la limite R&D ne réduit plus à tort la capacité physique RFN du tronçon.
- Une ligne conserve correctement ses propres sillons déjà affectés lors du calcul des contraintes.
- Correction du cas où une ligne avec des trains affectés pouvait afficher 0 voyageur transporté à cause d’une fréquence effective tombée à zéro.
...
- Génération et intégration des 10 visuels suivants du catalogue roulant, sans liseré décoratif, dans `Parc > Catalogue`, `Parc > Maintenance` et `Parc > Compositions`.
- Trains ajoutés : `TGV Sud-Est`, `TGV Atlantique`, `TGV Réseau`, `TGV Duplex`, `TGV POS`, `TGV Dasye`, `Euroduplex / 2N2`, `Thalys PBA/PBKA`, `Eurostar TMST` et `TGV M / Avelia Horizon`.
- Ajout des assets dédiés dans `public/assets/trains/high_speed/`.
- Extension du mapping client des illustrations pour afficher automatiquement ces 10 nouveaux visuels dans le parc.
- Conservation du format uniforme `13:8` et du cadrage harmonisé déjà en place sur les vignettes du parc.
- Schéma save : `184`.
...
- carte : un clic dans le vide désélectionne désormais la ligne précédemment sélectionnée ;
- carte : un clic sur une gare désélectionne aussi la ligne active avant d’ouvrir la fiche de gare ;
- conservation du comportement de clic direct sur une ligne : sélection, changement de ligne ou désélection si la ligne déjà focalisée est recliquée.
