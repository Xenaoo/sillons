...
function migrateState(loaded) {
  ...
  const migrated = {
    ...
    events: normalizeEvents(Array.isArray(loaded.events) ? loaded.events : []),
    nextEventAt: Number.isFinite(Number(loaded.nextEventAt)) ? Number(loaded.nextEventAt) : 0,
...
function createState() {
  return {
    ...
    market: createMarket(),
    events: [],
    nextEventAt: Date.now() + randomEventCooldownMs(true),
