...
function simulateTick() {
  ...
  scheduleMissingLineSpeedProfiles();
  updateMarket();
  updateEvents();
...
const pendingLineSpeedProfiles = new Set();

function scheduleMissingLineSpeedProfiles() {
  for (const player of activePlayers()) {
    for (const line of player.lines || []) {
      if (!line?.active || line?.speedProfile?.segments?.length) continue;
      const key = `${player.id}:${line.id}`;
      if (pendingLineSpeedProfiles.has(key)) continue;
      pendingLineSpeedProfiles.add(key);
      realRailRouteBetweenStops(lineStops(line))
        .then(routeInfo => {
          if (routeInfo?.speedProfile?.segments?.length) applyValidatedRouteToLine(line, routeInfo);
        })
        .catch(error => console.warn(`Profil de vitesse RFN indisponible pour ${line.id}:`, error.message))
        .finally(() => pendingLineSpeedProfiles.delete(key));
    }
  }
}
...
function passengerRunForTrain(train, line, stopCount, now) {
  ...
  train.passengerRun = next;
  return next;
}

function boardPassengersAtStop(run, stopIndex, direction, stopCount, capacity, demand) {
  const free = Math.max(0, Math.floor(capacity - run.load));
  const boarded = Math.max(0, Math.min(free, Math.floor(demand)));
  if (!boarded) return 0;
  const terminal = direction > 0 ? stopCount - 1 : 0;
  const nextStop = clamp(stopIndex + direction, 0, stopCount - 1);
  const shortTrip = Math.floor(boarded * 0.58);
  const longTrip = boarded - shortTrip;
  if (shortTrip && nextStop !== stopIndex) run.cohorts.push({ originIndex: stopIndex, destinationIndex: nextStop, count: shortTrip });
  if (longTrip && terminal !== stopIndex) run.cohorts.push({ originIndex: stopIndex, destinationIndex: terminal, count: longTrip });
  run.load += boarded;
  return boarded;
}

function scheduleTrainRunLeg(run, line, profile, cadence, stopCount, arrivedAt) {
  const fromIndex = clamp(Number(run.stopIndex || 0), 0, Math.max(0, stopCount - 1));
  const direction = Number(run.direction || 1) < 0 ? -1 : 1;
  const toIndex = clamp(fromIndex + direction, 0, Math.max(0, stopCount - 1));
  const travelMinutes = Math.max(0.05, lineLegTravelMinutes(line, fromIndex, toIndex, Number(profile?.speed || 0)));
  const dwellMinutes = fromIndex === 0 || fromIndex === stopCount - 1
    ? Math.max(0, Number(cadence?.turnaroundMinutes || 0))
    : Math.max(0, Number(cadence?.dwellMinutes || 0));
  const dwellMs = Math.round(dwellMinutes * 60000);
  const travelMs = Math.round(travelMinutes * 60000);
  run.lastStopAt = arrivedAt;
  run.dwellUntil = arrivedAt + dwellMs;
  run.departedAt = run.dwellUntil;
  run.legFromIndex = fromIndex;
  run.legToIndex = toIndex;
  run.legMs = travelMs;
  run.nextStopAt = run.departedAt + travelMs;
}

function processPassengerArrivals(player, line, trains, demandLoadFactor, ticketPrice, now, cadence) {
  const stops = lineStops(line);
  if (stops.length < 2 || !trains.length) {
    return { alighted: 0, boarded: 0, displayedAlighted: 0, revenue: 0, averageLoad: null, serviceCalls: 0, expectedServiceCalls: 0, arrivals: [] };
  }
  const passengerServiceEnabled = lineMarketServices(line).includes('passengers');
  const loadFactor = clamp(Number(demandLoadFactor || 0), 0, 1);
  let alighted = 0;
  let boarded = 0;
  let revenue = 0;
  let loadTotal = 0;
  let loadTrainCount = 0;
  ...
  for (const train of trains) {
    ...
    const demandPerStop = passengerServiceEnabled ? capacity * loadFactor : 0;
    const run = passengerRunForTrain(train, line, stops.length, now);
    if (!run.started) {
      const added = capacity > 0 ? boardPassengersAtStop(run, run.stopIndex, run.direction, stops.length, capacity, demandPerStop) : 0;
      run.lastBoarded = added;
      run.lastAlighted = 0;
      run.lastRevenue = 0;
      run.started = true;
      scheduleTrainRunLeg(run, line, profile, cadence, stops.length, now);
      boarded += added;
      if (capacity > 0) {
    ...
    while (now >= Number(run.nextStopAt || 0) && safety++ < 4) {
      const arrivedAt = Number(run.nextStopAt || now);
      const arrivingAt = clamp(Number(run.legToIndex ?? (Number(run.stopIndex || 0) + Number(run.direction || 1))), 0, stops.length - 1);
      let exitCount = 0;
      const remaining = [];
      run.lastBoarded = 0;
      run.lastAlighted = 0;
      ...
      for (const cohort of run.cohorts) {
        ...
        const count = Math.max(0, Math.floor(Number(cohort.count || 0)));
        const tripShare = Math.abs(Number(cohort.destinationIndex) - Number(cohort.originIndex)) / Math.max(1, stops.length - 1);
        const earned = count * ticketPrice * Math.max(0.18, tripShare);
        exitCount += count;
        revenue += earned;
        run.lastRevenue += earned;
      }
      run.cohorts = remaining;
      run.load = Math.max(0, run.load - exitCount);
      alighted += exitCount;
      ...
      const added = capacity > 0 ? boardPassengersAtStop(run, arrivingAt, run.direction, stops.length, capacity, demandPerStop) : 0;
      boarded += added;
      run.lastBoarded += added;
      scheduleTrainRunLeg(run, line, profile, cadence, stops.length, arrivedAt);
      serviceCalls += 1;
      arrivals.push({ trainId: train.id, stopIndex: arrivingAt, stationId: stops[arrivingAt] || null, boarded: added, alighted: exitCount });
    }
    if (capacity > 0) {
      loadTotal += Math.min(1, run.load / Math.max(1, capacity));
      loadTrainCount += 1;
  ...
  return {
    ...
    displayedAlighted,
    revenue,
    averageLoad: loadTrainCount ? round2(loadTotal / loadTrainCount * 100) : null,
    serviceCalls,
    expectedServiceCalls: expectedServiceCallsForCadence(cadence, stops.length, trains.length),
    arrivals
  };
}

function lineCirculationDemandFactor(cadence) {
  const speed = Math.max(0, Number(cadence?.operatingSpeedKmh || 0));
  // La demande produite par tick reflète le débit de la rame : à capacité égale,
  // un matériel plus rapide dessert davantage de gares sur une heure de jeu.
  return clamp(speed / 100, 0.45, 2.4);
}

function averagePassengerTripShare(stopCount) {
  const legs = Math.max(1, Number(stopCount || 0) - 1);
  return clamp(0.58 / legs + 0.42 * (legs + 1) / (2 * legs), 0.18, 1);
}

function expectedServiceCallsForCadence(cadence, stopCount, trainCount) {
  const legs = Math.max(1, Number(stopCount || 0) - 1);
  const roundTripMs = Math.max(1, Number(cadence?.roundTripMinutes || 0) * 60000);
  return Math.max(0, Number(trainCount || 0)) * legs * 2 * TICK_MS / roundTripMs;
}

function scalePassageRights(rights, factor) {
  const ratio = Math.max(0, Number(factor || 0));
  return {
    total: Math.max(0, Number(rights?.total || 0) * ratio),
    infrastructureTotal: Math.max(0, Number(rights?.infrastructureTotal || 0) * ratio),
    stationTotal: Math.max(0, Number(rights?.stationTotal || 0) * ratio),
    allocations: (rights?.allocations || []).map(allocation => ({
      ...allocation,
      amount: Math.max(0, Number(allocation?.amount || 0) * ratio)
    }))
  };
}

function computeAnnualLineCapacity(profile, cadence, service, capacityFactor) {
  const availableTrainCount = Math.max(0, Number(cadence?.availableTrainCount || 0));
  const activeTrainCount = Math.max(0, Number(cadence?.effectiveTrainCount || 0));
  const roundTripMinutes = Math.max(0, Number(cadence?.roundTripMinutes || 0));
  const operatingHoursPerDay = service === 'freight' ? 20 : service === 'mixed' ? 19 : 18;
  const annualRoundTripsPerTrain = roundTripMinutes > 0 ? operatingHoursPerDay * 60 * 365 / roundTripMinutes : 0;
  const activeFleetRatio = availableTrainCount > 0 ? clamp(activeTrainCount / availableTrainCount, 0, 1) : 0;
  const annualOneWayTrips = annualRoundTripsPerTrain * availableTrainCount * activeFleetRatio * 2;
  const usableFactor = clamp(Number(capacityFactor || 0), 0, 1);
  return {
    availableTrainCount: Math.round(availableTrainCount),
    activeTrainCount: round2(activeTrainCount),
    operatingHoursPerDay,
    roundTripMinutes: round2(roundTripMinutes),
    annualOneWayTrips: Math.round(annualOneWayTrips),
    passengers: Math.round(Math.max(0, Number(profile?.capacity || 0)) * annualRoundTripsPerTrain * activeFleetRatio * 2 * usableFactor),
    freightTons: Math.round(Math.max(0, Number(profile?.freight || 0)) * annualRoundTripsPerTrain * activeFleetRatio * 2 * usableFactor)
  };
}
...
function simulatePlayer(player, lineMarkets, passageRightsLedger = null, options = {}) {
  ...
  let passengers = 0;
  let freight = 0;
  let projectedStationPassengers = 0;
  let projectedStationFreight = 0;
  let expectedStationServiceCalls = 0;
  let actualStationServiceCalls = 0;
  let co2 = 0;
  ...
  for (const line of player.lines) {
    ...
    const annualPassengerLoad = annualCapacity.passengers > 0 ? round2(passengerAnnualTransport / annualCapacity.passengers * 100) : null;
    const annualFreightLoad = annualCapacity.freightTons > 0 ? round2(freightAnnualTransport / annualCapacity.freightTons * 100) : null;
    ...
    const expectedServiceCalls = expectedServiceCallsForCadence(cadence, stops.length, availableTrains.length);
    const passengerLoadFactor = maxPax > 0 ? clamp(linePax / maxPax, 0, 1) : 0;
    const freightLoadFactor = maxFreight > 0 ? clamp(lineFreight / maxFreight, 0, 1) : 0;
    const averagePassengerPerService = availableTrains.length > 0 ? operatingModel.capacity * passengerLoadFactor / availableTrains.length : 0;
    const averageFreightPerService = availableTrains.length > 0 ? operatingModel.freight * freightLoadFactor / availableTrains.length : 0;
    const projectedPassengerAlightings = expectedServiceCalls * averagePassengerPerService;
    const projectedFreightTons = expectedServiceCalls * averageFreightPerService;
    const passengerArrivals = dryRun
      ? {
          alighted: Math.round(projectedPassengerAlightings),
          boarded: Math.round(projectedPassengerAlightings),
          displayedAlighted: Math.round(projectedPassengerAlightings),
          revenue: 0,
          averageLoad: maxPax > 0 ? round2(passengerLoadFactor * 100) : null,
          serviceCalls: expectedServiceCalls,
          expectedServiceCalls,
          arrivals: []
        }
      : processPassengerArrivals(player, effectiveLine, availableTrains, passengerLoadFactor, effectiveTicketPrice, Date.now(), cadence);

    // Les montants affichés sont une moyenne fixe par tick. Elle dépend du
    // matériel réellement en circulation, de sa capacité et de sa vitesse
    // (via `circulationFactor` et la cadence), sans osciller entre deux gares.
    const projectedBaseTicketRevenue = projectedPassengerAlightings * effectiveTicketPrice * averagePassengerTripShare(stops.length) * profitabilityMultiplier * ECONOMY.passengerRevenueMultiplier;
    const projectedTicketRevenue = projectedBaseTicketRevenue * fareComplianceFactor;
    const projectedAncillaryRevenue = projectedPassengerAlightings * 0.35 * averageCommerce(player, line);
    const averageLegDistance = distance / legs;
    const freightRatePerKm = (0.045 + player.tech.freight * 0.003) * (operatingModel.freightRevenueMultiplier || 1) * profitabilityMultiplier * ECONOMY.freightRevenueMultiplier;
    const projectedFreightRevenue = projectedFreightTons * averageLegDistance * freightRatePerKm;
    const projectedServiceRevenue = projectedTicketRevenue + projectedAncillaryRevenue + projectedFreightRevenue;
    const lineRevenue = projectedServiceRevenue * dispatchRevenueFactor;

    // L'argent réel n'est encaissé qu'à une arrivée. Les recettes fret sont
    // distribuées sur ces mêmes dessertes pour ne plus créditer une rame entre
    // deux gares. La moyenne des encaissements rejoint la projection ci-dessus.
    const actualServiceRatio = dryRun ? 1 : passengerArrivals.serviceCalls / Math.max(0.0001, passengerArrivals.expectedServiceCalls || expectedServiceCalls);
    const actualBaseTicketRevenue = dryRun ? projectedBaseTicketRevenue : passengerArrivals.revenue * profitabilityMultiplier * ECONOMY.passengerRevenueMultiplier;
    const actualTicketRevenue = actualBaseTicketRevenue * fareComplianceFactor;
    const actualAncillaryRevenue = dryRun ? projectedAncillaryRevenue : passengerArrivals.alighted * 0.35 * averageCommerce(player, line);
    const actualFreightTons = dryRun
      ? projectedFreightTons
      : passengerArrivals.arrivals.reduce((sum, arrival) => {
          const servedTrain = availableTrains.find(item => item.id === arrival.trainId);
          const servedModel = servedTrain ? BALANCE.trains[servedTrain.modelId] : null;
          const servedProfile = servedTrain && servedModel ? getTrainOperatingProfile(servedTrain, servedModel, player) : null;
          return sum + Math.max(0, Number(servedProfile?.freight || 0)) * freightLoadFactor;
        }, 0);
    const actualFreightRevenue = actualFreightTons * averageLegDistance * freightRatePerKm;
    const actualServiceRevenue = actualTicketRevenue + actualAncillaryRevenue + actualFreightRevenue;
    const actualLineRevenue = actualServiceRevenue * dispatchRevenueFactor;
    ...
    const actualPassageRights = dryRun ? passageRights : scalePassageRights(passageRights, actualServiceRatio);
    const actualAccessCost = actualPassageRights.total;
    if (!dryRun) recordPassageRights(passageRightsLedger, player, line, actualPassageRights);
    const lineInfrastructureCost = computeLineInfrastructureCost(player, line, lineInfrastructureMultiplier, infrastructureUsage);
    const commercialBaseRevenue = projectedBaseTicketRevenue + projectedAncillaryRevenue + projectedFreightRevenue;
    ...
    const actualVariableExpenses = energyCost + maintenanceCost + actualAccessCost + lineInfrastructureCost + commercialOperatingCost;
    const lineCo2 = computeCo2(operatingModel, distance, serviceFactor);
    revenue += lineRevenue;
    cashRevenue += actualLineRevenue;
    expenses += variableExpenses;
    cashExpenses += actualVariableExpenses;
    ...
    line.stats = {
      passengers: Math.round(passengerAnnualTransport),
      freightTons: Math.round(freightAnnualTransport),
      ...
      co2PerHour: Math.round(lineCo2),
      capacity: {
        passengers: annualCapacity.passengers,
        freightTons: annualCapacity.freightTons,
        passengerLoad: annualPassengerLoad,
        passengerBoarded: Math.round(passengerArrivals.boarded),
        passengerAlighted: Math.round(passengerArrivals.displayedAlighted),
        serviceCalls: Math.round(passengerArrivals.serviceCalls || 0),
        expectedServiceCalls: round2(passengerArrivals.expectedServiceCalls || expectedServiceCalls),
        freightLoad: annualFreightLoad,
        availableTrainCount: annualCapacity.availableTrainCount,
        activeTrainCount: annualCapacity.activeTrainCount,
        annualOneWayTrips: annualCapacity.annualOneWayTrips,
        operatingHoursPerDay: annualCapacity.operatingHoursPerDay,
      ...
      finance: {
        ...
        ticketRevenue: Math.round(projectedTicketRevenue),
        ancillaryRevenue: Math.round(projectedAncillaryRevenue),
        freightRevenue: Math.round(projectedFreightRevenue),
        dispatchRevenueBoost: Math.round(Math.max(0, lineRevenue - projectedServiceRevenue)),
        actualTicketRevenue: Math.round(actualTicketRevenue),
        actualAncillaryRevenue: Math.round(actualAncillaryRevenue),
        actualFreightRevenue: Math.round(actualFreightRevenue),
        actualLineRevenue: Math.round(actualLineRevenue),
        ...
        actualAccessCost: Math.round(actualAccessCost),
        variableExpenses: Math.round(variableExpenses),
        actualVariableExpenses: Math.round(actualVariableExpenses),
    ...
    passengers += passengerArrivals.alighted;
    freight += actualFreightTons;
    projectedStationPassengers += projectedPassengerAlightings;
    projectedStationFreight += projectedFreightTons;
    expectedStationServiceCalls += passengerArrivals.expectedServiceCalls || expectedServiceCalls;
    actualStationServiceCalls += passengerArrivals.serviceCalls || 0;
  ...
  const actualStationRevenue = dryRun
    ? stationRevenue
    : stationRevenue * actualStationServiceCalls / Math.max(0.0001, expectedStationServiceCalls);
  ...
  cashRevenue += actualStationRevenue;
  ...
  cashExpenses += sharedCosts;

  const profit = cashRevenue - cashExpenses;
  const projectedProfit = revenue - expenses;
  if (dryRun) return { revenue, expenses, profit: projectedProfit, passengers, freight, co2 };
  player.cash += profit;
  ...
  player.stats.revenue += Math.round(cashRevenue);
  player.stats.expenses += Math.round(cashExpenses);
  ...
  player.stats.lastProfit = Math.round(projectedProfit);
...
function computeRouteDemand(from, to, line, player, eventFactor) {
  ...
  const fromDemand = stationAnnualPassengerDemand(from);
  const toDemand = stationAnnualPassengerDemand(to);
  const stopDemand = mids.reduce((sum, s) => sum + stationAnnualPassengerDemand(s) * 0.075, 0);
  // Les données SNCF mesurent des entrées/sorties en gare, pas des OD. La
  // demande d'un axe est donc une fraction pondérée des deux pôles, enrichie
  // par les arrêts intermédiaires, puis exprimée en voyageurs par an.
  const demandBase = Math.sqrt(fromDemand * toDemand) * 0.24 + stopDemand;
...
function computeOwnedStationRevenue(player, passengers, freightTons, serviceCalls = 0) {
  const assets = Object.values(player.stations || {});
  // Une gare ne crée plus de revenu passif : ses recettes sont encaissées à
  // l'occasion d'une desserte effective, jamais pendant la circulation.
  if (!assets.length || Number(serviceCalls || 0) <= 0) return 0;
